...
async function migrate() {
  ...
  const repairs = [
    ...
    { table: "printer_profiles", sql: `UPDATE printer_profiles SET paper_size = '80mm' WHERE paper_size NOT IN
      ('A4','A5','Letter','80mm','76mm','58mm')` },
  ];
  ...
  // Runs here, not in the early repairs list above — this genuinely depends
  // on at least one location existing, which is only guaranteed once the
  // seeding step above has run. Running it earlier meant it silently did
  // nothing on a database that reached that point with zero locations yet,
  // and never got a second chance once one was created moments later.
  const orphanFix = await pool.query(`
  ...
  `);
  if (orphanFix.rowCount > 0) console.log(`Assigned a branch to ${orphanFix.rowCount} product(s) that had none.`);
