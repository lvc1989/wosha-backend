...
CREATE TABLE IF NOT EXISTS users (
  ...
  layout_style TEXT NOT NULL DEFAULT 'sidebar' CHECK (layout_style IN ('sidebar','grid','grid-hamburger')),
  failed_login_attempts INT DEFAULT 0,
  locked_until TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT now()
);
ALTER TABLE users ADD COLUMN IF NOT EXISTS layout_style TEXT NOT NULL DEFAULT 'sidebar';
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_layout_style_check;
ALTER TABLE users ADD CONSTRAINT users_layout_style_check CHECK (layout_style IN ('sidebar','grid','grid-hamburger'));
