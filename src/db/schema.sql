...
UPDATE bookings SET owner_review_status = NULL WHERE owner_review_status IS NOT NULL
...
UPDATE bookings SET status = 'Requested' WHERE status NOT IN
...
UPDATE tasks SET status = 'Open' WHERE status NOT IN ('Open','In Progress','Submitted','Done','Rejected');
...
-- Widened to cover the additional real-world paper sizes added later (76mm,
-- A5, Letter) — any existing row with a value outside even the new list gets
-- normalized first, so this can never fail against real production data,
-- the same lesson learned from an earlier categories_type_check incident.
UPDATE printer_profiles SET paper_size = '80mm' WHERE paper_size NOT IN ('A4','A5','Letter','80mm','76mm','58mm');
ALTER TABLE printer_profiles DROP CONSTRAINT IF EXISTS printer_profiles_paper_size_check;
ALTER TABLE printer_profiles ADD CONSTRAINT printer_profiles_paper_size_check CHECK (paper_size IN ('A4','A5','Letter','80mm','76mm','58mm'));
ALTER TABLE printer_profiles ADD COLUMN IF NOT EXISTS connection TEXT DEFAULT 'usb';
ALTER TABLE printer_profiles ADD COLUMN IF NOT EXISTS network_address TEXT;
ALTER TABLE printer_profiles ADD COLUMN IF NOT EXISTS bluetooth_name TEXT;
...
-- Any row with a type value outside the allowed list (from old data, a manual
-- edit, or anything else this migration can't predict) gets normalized to
-- 'service' first — a real production deploy failed here once already because
-- an unexpected value made the constraint impossible to re-apply. Reassigning
-- a stray row's type is a far smaller risk than leaving the whole deploy broken.
...
ALTER TABLE categories ADD CONSTRAINT categories_type_check
  CHECK (type IN ('service','product','expense','purchase_order','client_segment','cashflow','supplier','task_template','branch_target','roster_activity'));
...
UPDATE custom_field_defs SET entity_type = 'customer' WHERE entity_type NOT IN ('customer','staff','booking','branch');
...
-- App-wide theme: two colors only, deliberately. Every icon/status color that
-- means something (danger=red, success=green) stays fixed regardless of theme —
-- only the two purely brand colors (the main accent and the secondary accent)
-- are customizable, so a theme change can never accidentally make "danger" look
-- like "success" or break what a color is supposed to communicate.
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS theme_primary_color TEXT DEFAULT '#2B6CF6';
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS theme_accent_color TEXT DEFAULT '#FFC93C';
-- The top strip (logo, branch picker, notifications, account) is visually
-- separate from the rest of the interface, so it gets its own independent
-- color rather than being forced to match the primary theme color.
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS theme_topbar_color TEXT DEFAULT '#FFFFFF';

-- Groups budget lines into collapsible categories (Overhead, Operations, etc.)
-- instead of one long flat list.
ALTER TABLE budget_lines ADD COLUMN IF NOT EXISTS group_name TEXT DEFAULT 'General';
UPDATE budget_lines SET group_name = 'Overhead' WHERE group_name = 'General' AND category IN
  ('Staff salaries & statutory contributions', 'Site rent / land lease', 'Insurance', 'Admin, licence renewals & misc.');
UPDATE budget_lines SET group_name = 'Operations' WHERE group_name = 'General' AND category IN
  ('Water (DAWASA + borehole upkeep)', 'Electricity (TANESCO)', 'Chemicals & consumables restock',
   'Generator diesel', 'Equipment maintenance & repairs', 'Waste/sludge disposal');
UPDATE budget_lines SET group_name = 'Marketing' WHERE group_name = 'General' AND category = 'Marketing & promotions';

-- ---------- Duty roster ----------
-- Distinct from manual_jobs (one-off ad-hoc tasks) — this is a real, date-based
-- staff activity schedule: who's doing what on which day, whether assigned by
-- the auto-scheduler or set manually, always overridable by a manager/GM/owner.
CREATE TABLE IF NOT EXISTS duty_roster (
  ...
  status TEXT DEFAULT 'Scheduled' CHECK (status IN ('Scheduled','In Progress','Done','Skipped')),
  auto_generated BOOLEAN DEFAULT false,
  notes TEXT,
  created_by UUID REFERENCES users(id),
  created_at TIMESTAMPTZ DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_duty_roster_date ON duty_roster(roster_date);
CREATE INDEX IF NOT EXISTS idx_duty_roster_technician ON duty_roster(technician_id);

-- One row, one switch: whether the auto-scheduler is allowed to run right now.
-- Manual roster creation/editing always works regardless of this setting.
CREATE TABLE IF NOT EXISTS roster_settings (
  id INT PRIMARY KEY DEFAULT 1 CHECK (id = 1),
  auto_scheduler_enabled BOOLEAN DEFAULT true
);
INSERT INTO roster_settings (id) VALUES (1) ON CONFLICT (id) DO NOTHING;

-- Reuses the existing categories system for the list of assignable activities
-- (Interior detail, Exterior wash, etc.) rather than a new bespoke table.
UPDATE categories SET type = 'service' WHERE type NOT IN
  ('service','product','expense','purchase_order','client_segment','cashflow','supplier','task_template','branch_target','roster_activity');
ALTER TABLE categories DROP CONSTRAINT IF EXISTS categories_type_check;
ALTER TABLE categories ADD CONSTRAINT categories_type_check CHECK (type IN
  ('service','product','expense','purchase_order','client_segment','cashflow','supplier','task_template','branch_target','roster_activity'));
INSERT INTO categories (type, name)
SELECT 'roster_activity', v FROM (VALUES
  ('Interior detailing'), ('Exterior wash'), ('Engine steam wash'),
  ('Tyre & rim shine'), ('Vacuuming'), ('Compliance & reporting'), ('Reception & customer service')
) AS v(v)
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE type = 'roster_activity');
...
-- Products with no branch assigned predate an earlier fix that made the
-- branch field required when adding a product — before that, every product
-- got location_id = NULL, which can never match ANY branch-filtered query,
-- making them permanently invisible in Sales regardless of which branch is
-- selected. Assigns them to the first real branch so they become visible and
-- sellable again; the owner can still reassign a specific one via Edit if
-- it actually belongs to a different branch.
UPDATE products SET location_id = (SELECT id FROM locations ORDER BY created_at ASC LIMIT 1)
WHERE location_id IS NULL AND EXISTS (SELECT 1 FROM locations);

-- ---------- Menu grid icon colors (separate from the general app theme) ----------
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS menu_icon_bg_color TEXT DEFAULT '#E6F1FB';
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS menu_icon_color TEXT DEFAULT '#185FA5';

-- ---------- Big hero-band customization (Dashboard greeting, Reports P&L tiles,
-- and every similar large band across the app) — background, font color, and a
-- transparency value that lets the ambient background circles show through more
-- or less, like a watermark.
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS hero_bg_color TEXT DEFAULT '#0B1B33';
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS hero_text_color TEXT DEFAULT '#FFFFFF';
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS hero_bubble_opacity NUMERIC(3,2) DEFAULT 0.18;

-- ---------- Printed header/footer font sizes (previously fixed) ----------
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS print_header_font_size INT DEFAULT 12;
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS print_footer_font_size INT DEFAULT 10;

-- ---------- Big stat-tile customization (Dashboard's four stat bars, Business
-- Plan's project figures, Reports' budget summary, and every other StatCard
-- app-wide) — nullable, so leaving these unset keeps each tile's own
-- meaningful tone (blue/navy/green/amber) exactly as before; setting them
-- overrides every tile uniformly, e.g. to the same blue/yellow rhythm used
-- elsewhere in Settings.
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS tile_bg_color TEXT;
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS tile_text_color TEXT;
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS tile_font_size INT;

-- ---------- Dashboard's four stat tiles — independently customizable ----------
-- Distinct from tile_bg_color/tile_text_color above (which apply one uniform
-- look to every StatCard app-wide) — this lets each of Dashboard's four
-- tiles (bookings, revenue, activeJobs, expenses) have its own background,
-- text color, and font size, while still sharing the same watermark-circle
-- treatment as the page header band.
ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS dashboard_tile_styles JSONB DEFAULT '{}';

-- A custom uploaded icon/image per service, overriding the automatic
-- keyword-matched icon when set.
ALTER TABLE services ADD COLUMN IF NOT EXISTS icon_url TEXT;

-- Promotional banners/ads for the public website — distinct from the photo &
-- video gallery (which showcases completed work), and distinct from the
-- existing "promotions" table (which is the app's internal discount-campaign
-- feature, not a website display). Named ad_banners specifically to avoid
-- colliding with that unrelated existing table.
...
-- Links an expense back to the purchase order it came from, when it was
-- created automatically from a received delivery (rather than logged by
-- hand) — so "purchase of 6 Dashboard Polish — TZS 48,000" can genuinely
-- show its real supplier and date on click, not text guessed back out of a note.
ALTER TABLE expenses ADD COLUMN IF NOT EXISTS purchase_order_id UUID REFERENCES purchase_orders(id);

-- Tracks the confirmed order/delivery instruction sent to the awarded
-- supplier once a PO is approved — genuinely distinct from send-quotation
-- (which goes to several candidate suppliers before one is picked) and from
-- delivery_note_url (which is what comes back FROM the supplier once they
-- actually deliver).
ALTER TABLE purchase_orders ADD COLUMN IF NOT EXISTS order_confirmation_sent_at TIMESTAMPTZ;
