...
router.get("/public", async (req, res) => {
  const { rows } = await pool.query(
    "SELECT business_name, address, phone, tagline, logo_url, login_message, login_background_color, loading_background_color, theme_primary_color, theme_accent_color, theme_topbar_color, menu_icon_bg_color, menu_icon_color, hero_bg_color, hero_text_color, hero_bubble_opacity, print_header_font_size, print_footer_font_size, tile_bg_color, tile_text_color, tile_font_size, dashboard_tile_styles, website_content FROM business_settings WHERE id = 1"
  );
  res.json(rows[0] || {});
});
...
router.put("/", requireAuth, requirePrimaryOwner, async (req, res) => {
  const { businessName, address, phone, tin, invoicePrefix, taxRatePercent, logoUrl, logoSize, tagline, staffAttachmentsEnabled, clientAttachmentsEnabled, mission, vision, orgStructure, sidebarColor, paymentInstructions, loginMessage, loginBackgroundColor, loadingBackgroundColor, iconBackgroundColor, printHeaderEnabled, printHeaderShowLogo, printHeaderShowSlogan, printHeaderAlign, printFooterEnabled, printFooterText, printFooterAlign, websiteContent, themePrimaryColor, themeAccentColor, themeTopbarColor, menuIconBgColor, menuIconColor, heroBgColor, heroTextColor, heroBubbleOpacity, printHeaderFontSize, printFooterFontSize, tileBgColor, tileTextColor, tileFontSize, dashboardTileStyles, resetDashboardTileKey } = req.body;
  const { rows } = await pool.query(
    `UPDATE business_settings SET
      ...
      dashboard_tile_styles = CASE WHEN $42::jsonb IS NOT NULL THEN dashboard_tile_styles || $42::jsonb ELSE dashboard_tile_styles END,
      updated_at = now()
     WHERE id = 1 RETURNING *`,
    [businessName, address, phone, tin, invoicePrefix, taxRatePercent, logoUrl, logoSize, tagline, staffAttachmentsEnabled, clientAttachmentsEnabled, mission, vision, orgStructure, sidebarColor, paymentInstructions, loginMessage, loginBackgroundColor, iconBackgroundColor, printHeaderEnabled, printHeaderShowLogo, printHeaderShowSlogan, printHeaderAlign, printFooterEnabled, printFooterText, printFooterAlign, websiteContent ? JSON.stringify(websiteContent) : null, loadingBackgroundColor, themePrimaryColor, themeAccentColor, themeTopbarColor, menuIconBgColor, menuIconColor, heroBgColor, heroTextColor, heroBubbleOpacity, printHeaderFontSize, printFooterFontSize, tileBgColor, tileTextColor, tileFontSize, dashboardTileStyles ? JSON.stringify(dashboardTileStyles) : null]
  );
  // JSONB's || merge operator can only add or overwrite a key, never remove
  // one — a "reset this tile" action needs the actual minus operator instead,
  // applied as a real second step so it always wins over any merge above.
  if (resetDashboardTileKey) {
    const { rows: resetRows } = await pool.query(
      "UPDATE business_settings SET dashboard_tile_styles = dashboard_tile_styles - $1 WHERE id = 1 RETURNING *",
      [resetDashboardTileKey]
    );
