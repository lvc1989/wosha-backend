...
router.get("/public", async (req, res) => {
  const { rows } = await pool.query(
    "SELECT business_name, logo_url, login_message, login_background_color FROM business_settings WHERE id = 1"
  );
  res.json(rows[0] || {});
});
...
router.put("/", requireAuth, requirePrimaryOwner, async (req, res) => {
  const { businessName, address, phone, tin, invoicePrefix, taxRatePercent, logoUrl, logoSize, tagline, staffAttachmentsEnabled, clientAttachmentsEnabled, mission, vision, orgStructure, sidebarColor, paymentInstructions, loginMessage, loginBackgroundColor, iconBackgroundColor } = req.body;
  const { rows } = await pool.query(
    ...
    [businessName, address, phone, tin, invoicePrefix, taxRatePercent, logoUrl, logoSize, tagline, staffAttachmentsEnabled, clientAttachmentsEnabled, mission, vision, orgStructure, sidebarColor, paymentInstructions, loginMessage, loginBackgroundColor, iconBackgroundColor]
  );
  res.json(rows[0]);
});
