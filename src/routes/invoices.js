...
// Looks up an invoice by its printed control number — used when scanning a physical
// receipt (or the control number printed on it as a barcode) to pull it back up.
router.get("/by-control-number/:code", requireAuth, async (req, res) => {
  const { rows } = await pool.query(`
    ...
    WHERE i.control_number = $1 GROUP BY i.id
  `, [req.params.code]);
  if (!rows[0]) return res.status(404).json({ error: "No invoice matches that control number." });
  res.json(rows[0]);
});
