...
router.get("/", requireAuth, async (req, res) => {
  ...
  // A plate scan that didn't match anyone creates a new customer automatically so the
  // sale isn't blocked — this is the follow-up nudge to go back and fill in their real
  // name, phone, etc. while it's still fresh, rather than it quietly staying incomplete.
  const newScannedCustomers = (await pool.query(
    "SELECT id, name FROM customers WHERE tag = 'New (Scanned)' AND phone IS NULL ORDER BY created_at DESC LIMIT 20"
  )).rows;
  newScannedCustomers.forEach((c) => items.push({ id: "new-customer-" + c.id, kind: "New Customer", label: `New customer from scan: ${c.name} — tap to complete their details`, urgent: false, customerId: c.id }));
