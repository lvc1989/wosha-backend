...
router.post("/", requireAuth, requireRole("owner", "manager"), async (req, res) => {
  const { name, type, paperSize, isDefault, connection, networkAddress, bluetoothName } = req.body;
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    if (isDefault) await client.query("UPDATE printer_profiles SET is_default = false");
    const { rows } = await client.query(
      "INSERT INTO printer_profiles (name, type, paper_size, is_default, connection, network_address, bluetooth_name) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *",
      [name, type || "Thermal", paperSize || "80mm", !!isDefault, connection || "usb", networkAddress || null, bluetoothName || null]
    );
