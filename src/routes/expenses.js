...
router.get("/", requireAuth, async (req, res) => {
  ...
  const conditions = [];
  const params = [];
  if (locationId && locationId !== "all") { params.push(locationId); conditions.push(`location_id = $${params.length}`); }
  ...
  const { rows } = await pool.query(`SELECT * FROM expenses ${where} ORDER BY expense_date DESC`, params);
  res.json(rows);
});

router.post("/", requireAuth, async (req, res) => {
  const { locationId, category, amount, note, expenseDate } = req.body;
  ...
  const { rows } = await pool.query(
    `INSERT INTO expenses (location_id, category, amount, note, status, submitted_by, expense_date)
     VALUES ($1,$2,$3,$4,$5,$6,COALESCE($7,CURRENT_DATE)) RETURNING *`,
    [locationId, category, amount, note, status, req.user.id, expenseDate]
  );
