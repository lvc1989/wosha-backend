...
router.post("/login", async (req, res) => {
  ...
  res.json({
    token,
    user: { id: user.id, name: user.name, role: user.role, locationId: user.location_id, profilePic: user.profile_pic },
  });
});
...
router.get("/me", requireAuth, async (req, res) => {
  const { rows } = await pool.query(
    "SELECT id, name, username, email, phone, role, location_id, profile_pic FROM users WHERE id = $1",
    [req.user.id]
  );
  res.json(rows[0]);
});
