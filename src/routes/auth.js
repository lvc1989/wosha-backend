...
router.post("/login", async (req, res) => {
  ...
  res.json({
    token,
    user: { id: user.id, name: user.name, role: user.role, title: user.title, isPrimaryOwner: user.is_primary_owner, locationId: user.location_id, profilePic: user.profile_pic, layoutStyle: user.layout_style },
  });
});
...
router.put("/profile", requireAuth, async (req, res) => {
  const { name, phone, email, profilePic, layoutStyle } = req.body;
  const { rows } = await pool.query(
    `UPDATE users SET name = COALESCE($1,name), phone = COALESCE($2,phone),
     email = COALESCE($3,email), profile_pic = COALESCE($4,profile_pic), layout_style = COALESCE($5,layout_style)
     WHERE id = $6 RETURNING id, name, username, email, phone, role, title, is_primary_owner, location_id, profile_pic, layout_style`,
    [name, phone, email, profilePic, layoutStyle, req.user.id]
  );
  const u = rows[0];
  res.json({ id: u.id, name: u.name, username: u.username, email: u.email, phone: u.phone, role: u.role, title: u.title, isPrimaryOwner: u.is_primary_owner, locationId: u.location_id, profilePic: u.profile_pic, layoutStyle: u.layout_style });
});
...
router.get("/me", requireAuth, async (req, res) => {
  const { rows } = await pool.query(
    "SELECT id, name, username, email, phone, role, title, is_primary_owner, location_id, profile_pic, layout_style FROM users WHERE id = $1",
    [req.user.id]
  );
  const u = rows[0];
